...
if(isset($_GET['action']) && $_GET['action']=='deconnexion'){// si le membre a cliqué sur déconnexion
    //debug($_SESSION);
    unset($_SESSION['membre']);//on ne fait pas un session destroy() qui supprimerait toute la session car on veut pouvoir
    //conserver les données du panier d'achat
    $message = '<div class="alert alert-info">Vous êtes déconnecté.</div>';
}

// 3 -Le membre déjà connecté est redirigé vers son profil:
if (estConnecte()){
    header('location:profil.php');// nous affichons le formulaire de connexion qu'aux membres non connectés. Les autres
    //sont redirigés vers le profil.
...
?>
    <form action="" method="post">
        ...
        <input type="submit" value="Se connecter" class="btn btn-info mt-2">
