...
$resultat = executeRequete("SELECT DISTINCT categorie FROM produit"); // DISTINCT pour dédoubolonner les catégories

$contenu_categories .= '<div class="list-group mb-4"></div>';
...
while($cat = $resultat->fetch(PDO::FETCH_ASSOC)) {
    debug($cat); // $cat est un tableau avec l'indice "categorie" qui contient une categorie différente par tour de boucle
...
if (isset ($_GET['categorie']) && $_GET['categorie'] != 'tous') {
    // si "categorie" est dans l'URL et que sa valeur est différente de "tous", alors on selectionne la catégorie demandée :
